...
const SessionCard = ({ session, onClick, onDelete }) => {
    ...
    const handleCardClick = () => {
        if (onClick) {
            ...
            } else if (session.status === 'in-progress' || session.status === 'pending') {
    ...
    const handleVideoInterview = (e) => {
        e.stopPropagation();
        navigate(`/video-interview/${session._id}`);
    };

    return (
        <div onClick={handleCardClick} className='group bg-white border border-slate-100 p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 transition-all hover:shadow-lg active:scale-[0.98] cursor-pointer'>
                ...
                <div className='flex flex-col items-end gap-1.5'>
                    ...
                    {session.status === 'in-progress' ? (
                        <button
                            ...
                            className='text-blue-600 font-bold text-xs flex items-center hover:text-blue-700 hover:underline'
                        >
                            🎥 Video
            ...
            <button onClick={(e) => { e.stopPropagation(); if (isDeletable && onDelete) onDelete(e, session._id) }} className='p-3 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all' title='Delete Session'>
